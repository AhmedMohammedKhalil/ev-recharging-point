...
if($method != "") {
    ...
    if($method == 'showChargingDetails') {
        $officeHour->showChargingDetails();
    }
...
Class OfficeHourController {
    ...
    private $Path = "../user/ChargingDetails/";


    public function showChargingDetails() {
        $join = "days as d ON oh.day_id = d.id";
        $where = "user_id = {$_SESSION['user']['id']}";
        $_SESSION['ChargingDetails'] = selectAll('oh.* , d.name as day','office_hours as oh',$where,$join);
    ...
    public function add() {
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST') { 
            if(isset($_POST['add_officeHour'])) {
                ...
                $subjects = selectAll('*','subjects',"user_id = $user_id and day_id = $day_id");
                if(!empty($subjects)) {
                ...
                $office_hours = selectAll('*','office_hours',"user_id = $user_id and day_id = $day_id");
    ...
    //             $office_hours = selectAll('*','office_hours',"user_id = $user_id and day_id = $day_id and id != $officeHour_id");
    //             if(!empty($office_hours)) {
    //                 foreach($office_hours as $s) {
    //                     if(strtotime($start_at) >= strtotime($s['start_at']) && strtotime($start_at) < strtotime($s['end_at'])) {
    //                         array_push($error,"The Office Houre is conflict with another office hour times");
    ...
    //             $success = update('day_id = ? , user_id = ? , start_at = ? , end_at = ? , slot_start = ? , slot_number = ?','office_hours',array_values($data),'id = ?');
    //             if($success) {
